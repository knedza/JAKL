...
specify_model <- function(data){
  ...
  results <- list('aic' = model$aic, 
                  'or_1' = exp(model$coefficients[2]), # odds ratios separated by JM 2019.06.25
                  'or_2' = exp(model$coefficients[3]), 
                  'or_3' = exp(model$coefficients[4]), 
                  'or_4' = exp(model$coefficients[5]), 
                  'or_5' = exp(model$coefficients[6]), 
                  'or_6' = exp(model$coefficients[7]), 
                  'p_1' = summary(model)$coefficients[2,4], # p values separated by JM 2019.06.25
                  'p_2' = summary(model)$coefficients[3,4], 
                  'p_3' = summary(model)$coefficients[4,4], 
                  'p_4' = summary(model)$coefficients[5,4], 
                  'p_5' = summary(model)$coefficients[6,4], 
                  'p_6' = summary(model)$coefficients[7,4], 
                  'ci_1' = c(ci[1],ci[7]),  # cis separated by JM 2019.06.25
                  'ci_2' = c(ci[2],ci[8]),  # returns a lo-hi pair per predictor level
                  'ci_3' = c(ci[3],ci[9]), 
                  'ci_4' = c(ci[4],ci[10]), 
                  'ci_5' = c(ci[5],ci[11]), 
                  'ci_6' = c(ci[6],ci[12]), 
